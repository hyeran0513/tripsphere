    ...
    <div
      ...
      <div className="pb-4 border-b border-stone-200 flex justify-between items-center">
        ...
        <button
          ...
          onClick={() => navigate(`/product/${data?.room?.accommodation_id}`)}>
      ...
      <div className="flex justify-between">
        <div className="flex gap-6 pt-6">
          <img
            ...
            src={data?.room?.images?.[0] || 'https://via.placeholder.com/100'}
            alt={data.accomData.name || '숙소 정보 없음'}
          />
          <div className="flex flex-col">
            <h2 className="text-lg font-bold">
              {data.accomData.name || '숙소 정보 없음'}
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <div className="mr-1 text-sm">
                성인: {data?.room?.capacity?.adults ?? 0}명, 미성년자:{' '}
                {data?.room?.capacity.children ?? 0}명
              </div>
            </div>
            ...
            <div className="flex items-center gap-10 mt-2">
              <div className="flex items-center gap-2">
                ...
                <span>{formatDate(data?.room?.check_in) || '날짜 없음'}</span>
              </div>
              <div className="flex items-center gap-2">
                <BiCalendarAlt />
                <span className="font-bold">체크아웃:</span>{' '}
                <span>{formatDate(data?.room?.check_out) || '날짜 없음'}</span>
              </div>
            </div>
          </div>
        </div>
